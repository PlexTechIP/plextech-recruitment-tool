...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const [session, member, banned] = await Promise.all([
    Session.findById(id).select('created_by status anonymous one_vouch_per_member focused_candidate_id focus_version').lean(),
  ...
  return NextResponse.json({
    session: { status: session.status, anonymous: session.anonymous, one_vouch_per_member: session.one_vouch_per_member, focused_candidate_id: session.focused_candidate_id?.toString() ?? null, focus_version: session.focus_version ?? 0 },
